...
interface FishCatchAggregateItem {
...
function readBaitStatistics(body: unknown): BaitStatisticsItem[] {
  ...
  return asArray(payload.items).map((value) => {
    const item = asObject(value);
    assert.deepEqual(Object.keys(item).sort(), ['bait', 'reportsCount']);
...
function readFishCatchAggregatePage(body: unknown): {
  items: FishCatchAggregateItem[];
  nextCursor: string | null;
} {
  const payload = asObject(body);
  assert.deepEqual(Object.keys(payload).sort(), ['items', 'nextCursor']);
...
async function createStatisticsReport(
  ...
  return prisma.catchReport.create({
    data: {
      ...
      weightGrams: overrides.weightGrams ?? 40,
...
void describe('CatchReport API (PostgreSQL e2e)', { concurrency: false }, () => {
  ...
  void test('parses and atomically creates ordered native batches without deduplicating reports', async () => {
    ...
    const [burbot, coho] = await Promise.all([
      prisma.fish.create({ data: { name: 'Налим', nameNormalized: 'налим' } }),
      prisma.fish.create({ data: { name: 'Кижуч', nameNormalized: 'кижуч' } }),
    ]);
    await prisma.bait.create({
      data: { name: 'Лягушка', nameNormalized: 'лягушка', type: 'BAIT' },
    });
    await prisma.fishingBaseFish.createMany({
      data: [
        { fishingBaseId: base.id, fishId: burbot.id },
        { fishingBaseId: base.id, fishId: coho.id },
      ],
    });

    const supplied = 'Налим 15,88 кг. Поймана на Амур: Понтонный мост, Лягушка.(4.65 над леской)';
    const second = 'Кижуч 7,242 кг. Поймана на Амур: Понтонный мост, Лягушка.';
    const source = `${supplied}\r\n\n${second}\r${supplied}`;

    await mutation(api().post('/api/v1/catch-reports/parse-batch'))
      .send({ rawSourceText: source })
      .expect(401);
    const parsedResponse = await mutation(
      api().post('/api/v1/catch-reports/parse-batch'),
      actor.cookie,
    )
      .send({ rawSourceText: source })
      .expect(200);
    const rows = asArray(asObject(parsedResponse.body as unknown).rows).map(asObject);
    assert.equal(rows.length, 3);
    assert.deepEqual(
      rows.map((row) => [row.index, row.sourceLine, asObject(row.draft).rawSourceText]),
      [
        [0, 1, supplied],
        [1, 3, second],
        [2, 4, supplied],
    ...
    assert.deepEqual(rows[0]?.duplicateIndexes, [2]);
    assert.deepEqual(rows[1]?.duplicateIndexes, []);
    assert.deepEqual(rows[2]?.duplicateIndexes, [0]);
    assert.equal(await prisma.catchReport.count(), 0);

    const reports = rows.map((row) => {
      const draft = asObject(row.draft);
      const fields = asObject(draft.fields);
      const field = (name: string) => asObject(fields[name]);
      const itemId = (name: string) => asString(asObject(field(name).value).id, `${name}.id`);
      return {
        ...
        baitId: itemId('bait'),
        weightGrams: asNumber(field('weightGrams').value, 'weightGrams'),
        holeDepthCm: field('holeDepthCm').value,
        spotPositionRaw: field('spotPositionRaw').value,
        fishingNote: field('fishingNote').value,
        spinningSize: field('spinningSize').value,
        spinningSpeed: field('spinningSpeed').value,
    ...
    const createdResponse = await mutation(api().post('/api/v1/catch-reports/batch'), actor.cookie)
      .send({ reports })
      .expect(201);
    const created = asObject(createdResponse.body as unknown);
    assert.deepEqual(Object.keys(created).sort(), ['createdCount', 'reportIds']);
    assert.equal(created.createdCount, 3);
    const reportIds = asArray(created.reportIds).map((id) => asString(id, 'reportId'));
    assert.equal(new Set(reportIds).size, 3);

    const storedInRequestOrder = await Promise.all(
      reportIds.map((id) => prisma.catchReport.findUniqueOrThrow({ where: { id } })),
    );
    assert.deepEqual(
      storedInRequestOrder.map((report) => report.rawSourceText),
      [supplied, second, supplied],
    );
    assert.deepEqual(
      new Set(storedInRequestOrder.map((report) => report.contributorKey)),
      new Set([nativeContributorKey(actor.userId)]),
    );
    assert.ok(storedInRequestOrder.every((report) => report.importKey === null));

    const invalidBatch = await mutation(api().post('/api/v1/catch-reports/batch'), actor.cookie)
      .send({ reports: [reports[0], { ...reports[1], fishId: randomUUID() }] })
      .expect(409);
    assert.equal(readErrorCode(invalidBatch.body as unknown), 'CATCH_REPORT_BATCH_INVALID');
    const invalidErrors = asObject(asObject(invalidBatch.body as unknown).errors);
    assert.ok(Array.isArray(invalidErrors['reports.1.fishId']));
    assert.equal(await prisma.catchReport.count(), 3);

    const aggregate = readFishCatchAggregatePage(
      (
        await api()
          .get('/api/v1/catch-reports/statistics/fish-catches')
          .query({ fishId: burbot.id, baseIds: base.id, limit: 100 })
          .expect(200)
      ).body as unknown,
    );
    assert.equal(aggregate.items.length, 1);
    assert.equal(aggregate.items[0]?.intensity, 2);
    assert.equal(aggregate.items[0]?.contributorCount, 1);
    ...
    await mutation(api().post('/api/v1/catch-reports/parse-batch'), actor.cookie)
      .send({ rawSourceText: supplied })
      .expect(200);
    const bannedCreate = await mutation(api().post('/api/v1/catch-reports/batch'), actor.cookie)
      .send({ reports: [reports[0]] })
      .expect(403);
  ...
  void test('limits parse and create batches to 5000 reports', async () => {
    ...
    const parse = await mutation(api().post('/api/v1/catch-reports/parse-batch'), actor.cookie)
      .send({ rawSourceText })
      .expect(400);
  ...
  void test('Fish catch aggregates collapse observations and paginate by the complete stable order', async () => {
    const firstCatalog = await createCatalog();
    const secondCatalog = await createCatalog();
    const lowerBait = await createBait('LURE');
    const tiedBait = await createBait('BAIT');
    const [firstActor, secondActor] = await Promise.all([createActor(), createActor()]);
    const secondLocation = await prisma.location.create({
      data: {
        fishingBaseId: firstCatalog.base.id,
        number: 2,
        name: 'Aggregate Location Two',
        nameNormalized: 'aggregate location two',
      },
    });
    await prisma.fishingBase.update({
      where: { id: firstCatalog.base.id },
      data: { name: 'Aggregate Alpha Base', nameNormalized: 'aggregate alpha base' },
    });
    await prisma.fishingBase.update({
      where: { id: secondCatalog.base.id },
      data: { name: 'Aggregate Beta Base', nameNormalized: 'aggregate beta base' },
    });
    await prisma.bait.update({
      where: { id: firstCatalog.bait.id },
      data: { name: 'Aggregate Z Bait', nameNormalized: 'aggregate z bait', isActive: false },
    });
    await prisma.bait.update({
      where: { id: lowerBait.id },
      data: { name: 'Aggregate A Bait', nameNormalized: 'aggregate a bait' },
    });
    await prisma.bait.update({
      where: { id: tiedBait.id },
      data: { name: 'Aggregate B Bait', nameNormalized: 'aggregate b bait' },
    });
    ...
    do {
      const page = readFishCatchAggregatePage(
        (
          await api()
            .get(endpoint)
            .query({ ...scope, limit: 1, ...(cursor === null ? {} : { cursor }) })
            .expect(200)
        ).body as unknown,
      );
    ...
    assert.deepEqual(pagedItems, fullPage.items);

    const invalidCursor = await api()
      .get(endpoint)
      .query({ ...scope, cursor: 'not-json' })
      .expect(400);
  ...
  void test('Bait statistics respects selected/all Bases and collapses methods by Bait', async () => {
    ...
    for (const method of ['BAIT_FISHING', 'SPINNING', 'SPINNING'] as const) {
      await createStatisticsReport(actor, firstCatalog, {
        fishingMethod: method,
        ...(method === 'SPINNING'
          ? { holeDepthCm: null, spinningSize: 'MEDIUM' as const, spinningSpeed: 'SLOW' as const }
          : {}),
      });
    }
    await createStatisticsReport(actor, firstCatalog, { baitId: secondaryBait.id });
    await createStatisticsReport(actor, firstCatalog, { baitId: secondaryBait.id });
    for (let index = 0; index < 4; index += 1) {
      await createStatisticsReport(actor, secondCatalog, {
        fishId: firstCatalog.fish.id,
        baitId: firstCatalog.bait.id,
      });
    }

    const firstBaseItems = readBaitStatistics(
      (
        await api()
          .get(endpoint)
          .query({ fishId: firstCatalog.fish.id, baseIds: firstCatalog.base.id })
    ...
    assert.deepEqual(
      firstBaseItems.map((item) => [item.bait.id, item.reportsCount]),
      [
        [firstCatalog.bait.id, 3],
        [secondaryBait.id, 2],
      ],
    );

    const secondBaseItems = readBaitStatistics(
      (
        await api()
          .get(endpoint)
    ...
    assert.deepEqual(
      secondBaseItems.map((item) => [item.bait.id, item.reportsCount]),
  ...
  void test('Bait statistics reflects report edits and deletes immediately', async () => {
    ...
    assert.deepEqual(
      initial.map((item) => [item.bait.id, item.reportsCount]),
      [[catalog.bait.id, 2]],
    );
    ...
    await mutation(api().patch(`/api/v1/catch-reports/${editedReportId}`), editor.cookie)
      .send({ baitId: lure.id, holeDepthCm: null, spinningSize: 'SMALL', spinningSpeed: 'FAST' })
      .expect(200);
    ...
    assert.deepEqual(
      afterEdit.map((item) => [item.bait.id, item.reportsCount]).sort(),
      [
        [catalog.bait.id, 1],
        [lure.id, 1],
      ].sort(),
    ...
    assert.deepEqual(
      afterDelete.map((item) => [item.bait.id, item.reportsCount]),
      [[lure.id, 1]],
    );
  });
  ...
  void test('counts opaque external contributors independently for one ADMIN owner', async () => {
    ...
    assert.deepEqual(
      baitItems.map((item) => item.reportsCount),
      [5],
    );
