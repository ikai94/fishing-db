...
Отдельная явная команда добавляет базу `Амур`, 9 локаций, 64 глобальные рыбы, 64 связи
FishingBase–Fish и 8 экранных ориентиров. Наживки этот seed не выдумывает:

```powershell
pnpm db:seed:catalog
```

Seed сначала полностью валидирует набор данных, затем выполняет одну транзакцию. Он повторно
использует существующие нормализованные сущности, не переименовывает, не реактивирует, не деактивирует и не
удаляет ручные/посторонние строки. Структурный конфликт откатывает всю транзакцию. `db:seed:catalog` не входит в
`test`, `test:e2e` или `check`; ADMIN-bootstrap по-прежнему запускается отдельно через `pnpm db:seed`.
