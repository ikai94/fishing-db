...
Для новой задачи Codex обычно начинает с:

1. `AGENTS.md`;
2. `docs/PROJECT_STATE.md`;
3. `docs/ARCHITECTURE.md`;
4. `pnpm verify:state`;
5. `git diff` относительно принятого baseline;
6. просмотра только затронутых модулей.

Чистый принятый commit — доверенный baseline. Не следует повторно аудировать неизменённые принятые
этапы, если новая задача не затрагивает их инварианты; отчёт должен описывать только новый delta.
