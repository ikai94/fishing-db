...
Для проектирования изменения без правок используйте skill `fishing-design`; для реализации уже
согласованного изменения — `fishing-implementation`; для одной финальной проверки стабильного
результата — `fishing-acceptance`.

Все три workflow соблюдают `AGENTS.md`, запускают `pnpm verify:state`, читают
`docs/PROJECT_STATE.md` и загружают scoped AGENTS только для реально затронутых директорий.
`docs/ARCHITECTURE.md` читается лишь когда нужна навигация, трассировка request flow или обзор
нескольких слоёв; для локальной задачи с известными файлами он пропускается. `pnpm verify:state`
задаёт текущий Git baseline, а `docs/PROJECT_STATE.md` — принятое функциональное состояние.
Неизменённые принятые этапы не переаудируются без причины; отчёт описывает только новый delta.
...
Минимум отчёта: Fish, положительный вес, Location с производной Base, Bait/Lure и действующая
связь Base–Fish. Глубина, позиция, условие ловли, размер/скорость спиннинга и публичный
`userNoteRaw` («Комментарий») опциональны и хранятся раздельно. BAIT не принимает размер или
скорость спиннинга; SPINNING может не содержать один или оба этих параметра. Подтверждённая
строка блокнота сохраняется точно в owner-only `rawSourceText` и недоступна через PATCH.
